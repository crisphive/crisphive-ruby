  ...
  class JobRequestCreateRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Crisphive::JobRequestCreateRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Crisphive::JobRequestCreateRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'job_dates')
        if (value = attributes[:'job_dates']).is_a?(Array)
          self.job_dates = value
        end
      else
        self.job_dates = nil
      end
    ...
    def list_invalid_properties
      ...
      if @job_dates.nil?
        invalid_properties.push('invalid value for "job_dates", job_dates cannot be nil.')
      end

      if @job_dates.length > 12
        invalid_properties.push('invalid value for "job_dates", number of items must be less than or equal to 12.')
      end

      if @job_dates.length < 1
        invalid_properties.push('invalid value for "job_dates", number of items must be greater than or equal to 1.')
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] job_dates Value to be assigned
    def job_dates=(job_dates)
      if job_dates.nil?
        fail ArgumentError, 'job_dates cannot be nil'
      end

      if job_dates.length > 12
        fail ArgumentError, 'invalid value for "job_dates", number of items must be less than or equal to 12.'
      end

      if job_dates.length < 1
        fail ArgumentError, 'invalid value for "job_dates", number of items must be greater than or equal to 1.'
      end
