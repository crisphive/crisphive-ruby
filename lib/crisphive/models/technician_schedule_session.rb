...
module Crisphive
  class TechnicianScheduleSession
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Crisphive::TechnicianScheduleSession` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Crisphive::TechnicianScheduleSession`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      role_validator = EnumAttributeValidator.new('String', ["lead", "crew"])
    ...
    def role=(role)
      validator = EnumAttributeValidator.new('String', ["lead", "crew"])
    ...
    def hash
      [end_at, job_id, priority, role, short_code, start_at].hash
    end
