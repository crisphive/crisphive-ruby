  ...
  class CustomerUpdateRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Crisphive::CustomerUpdateRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Crisphive::CustomerUpdateRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'preferred_technician_id')
        self.preferred_technician_id = attributes[:'preferred_technician_id']
      end
    ...
    def list_invalid_properties
      ...
      if !@uid.nil? && @uid.to_s.length > 32
        invalid_properties.push('invalid value for "uid", the character length must be smaller than or equal to 32.')
      end
    ...
    def valid?
      ...
      return false if !@uid.nil? && @uid.to_s.length > 32
      true
    end
    ...
    def uid=(uid)
      ...
      if uid.to_s.length > 32
        fail ArgumentError, 'invalid value for "uid", the character length must be smaller than or equal to 32.'
      end
