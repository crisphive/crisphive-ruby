  ...
  class JobRequestSession
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Crisphive::JobRequestSession` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Crisphive::JobRequestSession`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'depart_at')
        self.depart_at = attributes[:'depart_at']
      end
      ...
      if attributes.key?(:'return_at')
        self.return_at = attributes[:'return_at']
      end
    ...
    def hash
      [date, depart_at, end_at, ordinal, return_at, start_at, travel_minutes].hash
    end
